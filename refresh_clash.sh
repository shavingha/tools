...
# 订阅脚本及地址（通过环境变量提供，不再写入仓库）
PYTHON_BIN="${PYTHON_BIN:-python3}"
SUBSCRIPTION_SCRIPT="${SUBSCRIPTION_SCRIPT:-${SCRIPT_DIR}/subscription_to_clash.py}"
SUBSCRIPTION_URL_PRIMARY="${SUBSCRIPTION_URL_PRIMARY:-}"
SUBSCRIPTION_URL_FALLBACK="${SUBSCRIPTION_URL_FALLBACK:-}"
GENERATED_CONFIG_PATH="${GENERATED_CONFIG_PATH:-${SCRIPT_DIR}/config.local.yaml}"
...
refresh_subscription() {
    # 调用转换脚本刷新订阅，主备地址均来自环境变量
    if [[ ! -x "${SUBSCRIPTION_SCRIPT}" && ! -f "${SUBSCRIPTION_SCRIPT}" ]]; then
        err "未找到订阅脚本: ${SUBSCRIPTION_SCRIPT}"
        return 1
    fi

    if [[ -n "${SUBSCRIPTION_URL_PRIMARY}" ]]; then
        log "尝试主订阅地址刷新"
        if "${PYTHON_BIN}" "${SUBSCRIPTION_SCRIPT}" "${SUBSCRIPTION_URL_PRIMARY}" -o "${GENERATED_CONFIG_PATH}"; then
            log "主订阅刷新成功"
            return 0
        fi
    fi

    if [[ -n "${SUBSCRIPTION_URL_FALLBACK}" ]]; then
        err "主订阅刷新失败，尝试备用订阅地址"
        if "${PYTHON_BIN}" "${SUBSCRIPTION_SCRIPT}" "${SUBSCRIPTION_URL_FALLBACK}" -o "${GENERATED_CONFIG_PATH}"; then
            log "备用订阅刷新成功"
            return 0
        fi
    fi

    err "未配置可用的订阅地址，或刷新全部失败"
    return 1
}
...
start_clash_background() {
    ...
    if [[ -z "${effective_config}" ]]; then
        ...
        elif [[ -f "${GENERATED_CONFIG_PATH}" ]]; then
